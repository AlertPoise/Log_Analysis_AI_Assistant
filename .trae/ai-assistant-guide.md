...
当前 behavior/UEBA 模块如存在专用文档，应以 .trae/behavior/ 下的模块 prompt 为准。

当前 behavior-new 分支的 UEBA 第一版目标是离线一次性用户行为基线构建。当前第一版不做实时动态基线更新、Kafka 实时消费、完整异常评分闭环。

通用指南中关于动态基线、实时检测、完整异常检测链路的内容属于后续扩展方向。
...
3. 🔲 **报告生成** - 实现日报和周报自动生成
4. 🔲 **威胁分类** - 完善威胁类型体系
5. 🔲 **行为基线** - 实现动态基线更新（后续扩展方向，当前 UEBA 第一版不实现）
