...
当根目录通用规范与模块目录下的 prompt 冲突时：

1. 用户当前明确要求优先级最高。
2. 当前开发模块目录下的 prompt 是模块级主要依据。
3. 根目录文档只提供通用规范。
4. quick-reference.md 只作速查参考。
5. task-checklist.md 只作历史进度参考，不能覆盖用户当前明确目标。
6. 实际开发前必须以当前真实文件树确认文件是否存在。
...
- 只输出修改的部分，不要输出完整文件
- 优先做最小必要改动，避免大范围重写
- 可以使用编辑器、补丁或 Codex 支持的文件修改方式，但必须保证 diff 清晰可审查
- 对于复杂改动，先说明方案再实现
- 保持代码简洁，删除无用的注释和代码
...
- 未经用户明确要求，不自动执行 git commit。
- 未经用户明确要求，不自动执行 git push。
- 修改完成后先汇报 modified files、测试结果、git diff 摘要。
- 用户确认后再提交。
- commit message 使用中文。
