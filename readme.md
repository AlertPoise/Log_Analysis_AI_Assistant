...
│   │   ├── conftest.py
│   │   ├── run_behavior.sh
│   │   ├── run_behavior_frontend_api_test.sh
│   │   ├── test_behavior.py
│   │   ├── test_behavior_api.py
...
- `src/behavior/service.py`：负责统一行为分析服务入口。
- `src/behavior/api.py`：负责面向前端的行为分析适配接口。

说明：
- `tests/behavior/run_behavior.sh` 和 `tests/behavior/behavior_config_cli.py` 是本地辅助脚本，不是核心 pytest 入口。
- `tests/behavior/run_behavior_frontend_api_test.sh` 用于前端接口适配联调。
- behavior 核心 pytest 用例主要依赖 fixture 构造样本，不要求外部服务已启动。
