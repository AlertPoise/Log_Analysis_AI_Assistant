...
阶段说明：当前 behavior-new 分支的 UEBA 第一版阶段目标以 .trae/behavior 下的模块 prompt 为准。当前第一版只实现离线一次性用户行为基线构建：从结构化日志数据库中按时间范围读取聚合结果、按用户生成行为基线、写入基线表。README 中涉及动态基线、实时检测、Flink、完整异常评分闭环的内容属于项目最终愿景或后续阶段，不作为当前第一版必须实现内容。
...
阶段说明：当前 UEBA 第一版只实现离线一次性用户行为基线构建，具体以 .trae/behavior 下的模块 prompt 为准；定时更新基线模型、动态基线和实时检测属于后续阶段。
...
阶段说明：这里的异常检测规则和完整异常评分闭环属于项目最终愿景或后续阶段；当前 UEBA 第一版只产出离线用户行为基线。
...
阶段说明：实时告警、Flink 实时异常检测和自定义异常规则属于后续阶段，不作为当前 behavior-new 分支 UEBA 第一版必须实现内容。
...
- **日志处理与存储**: Flink + Kafka + ClickHouse/Elasticsearch
  - 阶段说明：Flink 属于项目最终愿景或后续实时处理阶段；当前 UEBA 第一版以离线一次性基线构建为准。
...
阶段说明：当前 behavior-new 分支的 UEBA 第一版以 .trae/behavior 下的模块 prompt 为准，仅实现从结构化日志数据库读取聚合结果、按用户生成行为基线并写入基线表；本节中旧文件名或异常检测描述不覆盖当前第一版目标。
