...
5. 查看整体状态
6. 一键执行基础准线完整流程
7. 一键持续流量联动验收（需 YES + DELETE 二次确认）
8. 退出
9. 全流程闭环（3→6→7）
```
...
所有真实写库操作执行前必须输入 `YES`（大小写不敏感，yes/YES/Yes 均可；DELETE/delete/Delete 同理）：
...
**根菜单 6**：一键基础准线完整流程。按序执行 generate-expected → load-fixture → build-baseline → validate-baselines，任一步失败即停止。使用非交互 Runner flags。

输入不是 `YES`（大小写不敏感）或遇到 EOF → 取消操作，不写库，不 cleanup。
...
根菜单 3/6/7/9 和子菜单 12 已启用：
- **根菜单 3**：训练表更新闭环（5月初始化→基线构建→6月替换→不变校验→重建→diff）
- **根菜单 6**：一键基础准线完整流程（使用非交互 Runner flags，YES 大小写不敏感确认）
- **根菜单 7 / 子菜单 12**：一键持续流量联动验收（YES 确认写库 + DELETE 二次确认 cleanup，timeout 保护）
- cleanup helper 已从 username LIKE 加固为 username = 精确匹配
- **根菜单 9**：全流程闭环（3→6→7 按序执行，YES 确认）
