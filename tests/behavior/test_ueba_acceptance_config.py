...
def test_acceptance_config_defaults_are_safe_and_sized():
    ...
    assert 10 <= config.expected_user_count <= 35
    ...
    assert config.validation_baseline_user_count == 2
    assert config.expected_user_count == 28
    assert config.total_expected_logs == 72130
